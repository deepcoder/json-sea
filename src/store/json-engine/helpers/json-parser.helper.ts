...
export const jsonParser = (
    ...
    } = {isMongoData: false, skipRootEdges: false}
): {
    seaNodes: SeaNode[];
    edges: Edge[];
} => {
    ...
    const addDefaultEdge = ({source, target, sourceHandle}: DefaultEdgeParams): void => {
        // Skip edge creation from root for MongoDB data
        if (options.isMongoData && source === formatNodeId(0)) {
            return;
        }

        // Skip all root edges if explicitly specified
        if (options.skipRootEdges && source === formatNodeId(0)) {
            return;
        }
